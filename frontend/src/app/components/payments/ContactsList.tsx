// components/common/ContactList.tsx
...
const ContactList: React.FC<ContactListProps> = ({ contacts, onContactSelect }) => {
  ...
  return (
    <div className='flex flex-col gap-y-4'>
      ...
      <div className="w-full flex-col flex gap-y-4 overflow-y-auto" style={{ maxHeight: '400px' }}>
        <ul className="grid grid-cols-1 gap-4">
          {contacts.map((contact) => (
            <li
              key={contact.id}
              onClick={() => handleContactClick(contact.id)}
              className="bg-white p-4 rounded-lg shadow-md flex flex-row items-center cursor-pointer"
            >
